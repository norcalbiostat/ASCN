...
This chapter demonstrates different data visualizations by variable type (categorical, continuous, or both) and number of variables. Most visualizations are created using multiple techniques to demonstrate different package features and approaches. 


## Choosing your audience

Before you start visualizing your data, you should first determine who your audience is. The level of detail you put into your graphs/tables will depend on who the graph/table is for. In general, you will be visualizing data for
...
A common way to display categorical data frequencies is a barchart/barplot where the bars are typically drawn along the X-axis where the height of the bars is determined by the frequencies seen in the table. 
...
Often you don't want to compare counts but percents. Many statistical tests compare percentages, not counts. 

::: {.panel-tabset}

## ggplot

To accomplish this in ggplot we have to aggregate the data to calculate the proportions first, then plot the aggregated data using `geom_col` to create the columns.
...
### Other ways to visualize categorical data
...
The base function `plot()` creates a **dotplot** for a continuous variable. The value of the variable is plotted on the y axis, and the index, or row number, is plotted on the x axis. This gives you a nice, quick way to see the distribution of _individual_ values in the data. 
...
## base
[There are missing values in `bill_length_mm` and so I had to get rid of it using `na.omit()` prior to creating the density plot. See @sec-missing_fix for details]{.aside}
```{r}
...
### Mosaic plots
But what if you want to know how two categorical variables are related and you don't want to look at two different barplots? Mosaic plots are a way to visualize the proportions in a table. So here's the two-way table we'll be plotting - number of cylinders (4, 6, and 8) compared to number of gears (3, 4, and 5) in the `mtcars` dataset. 
...
```{r, fig.width=10}
#| layout-ncol: 2
mosaicplot(cyl~gear, data=mtcars, color=TRUE)
mosaicplot(cyl~gear, data=mtcars, shade=TRUE)
```

For example, there are fewer 4- and 6-cylinder cars with three gears and more 8-cylinder cars with three gears than expected. As you can see, knowing what your data means when trying to interpret what the plots are telling you is essential. 
...
Let's look at trends over time in the body mass of penguins. First we calculate the average body mass per year. 

```{r}
body.changes.overtime <- pen %>% group_by(year) %>% 
...
```{r}
plot(mean~year, data=body.changes.overtime, type='l')
```

## ggplot
With ggplot we specify that we want a line geometry only. 
```{r}
ggplot(body.changes.overtime, aes(x=year, y=mean)) + geom_line()
```

:::

How does this relationship change with penguin species? This is much easier completed in ggplot than base R, so only that method is shown.

First lets get the average body mass per year and species 

```{r}
species.mass.overtime <- pen %>% group_by(species, year) %>% 
  summarise(mean = mean(body_mass_g, na.rm=TRUE))
```
...
```{r}
ggplot(species.mass.overtime, aes(x=year, y=mean, col=species)) + geom_line()
```
...
conveys all the necessary information for your graph.
